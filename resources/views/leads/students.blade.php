...
<div class="card m-0 p-0">
    <div class="card-header" style="padding: 1%;">
        ...
        <div class="header-actions">
            ...
            <button class="dt-button add-new btn btn-primary ms-n1" tabindex="0" aria-controls="DataTables_Table_0" type="button" id="AddStudent">
                <span><i class="bx bx-plus me-0 me-lg-2"></i><span class="d-none d-lg-inline-block">Add New Student</span></span>
            </button>
        </div>
    </div>
      ...
      <table class="table table-hover table-bordered mb-0">
        <thead class="table-light">
            <tr>
                ...
                <th class="p-2 col-0">StuID</th>
                <th class="p-2 col-1">Name</th>
                <th class="p-2 col-3">Email</th>
                <th class="p-2 col-2">Country</th>
                <th class="p-2 col-3">Phone</th>
                <th class="p-2 col-2">Last Degree</th>
        ...
        <tbody>
            ...
            @else
                ...
                <tr>
                    <td class="p-1"> <img src="{{ $student->photo ? asset('storage/' . $student->photo) : 'https://ui-avatars.com/api/?name=' . urlencode($student->name) }}" 
                        alt="{{ $student->name }}" 
                        class="img-fluid rounded-circle"
                        style="width: 30px; height: 30px; object-fit: cover;"></td>

                    <td class="p-1">{{ $student->student_id }}</td>
                    <td class="p-1">{{ $student->name }}</td>
                    <td class="p-1">{{ $student->email }}</td>
                    <td class="p-1">{{ $student->country }}</td>    
                    <td class="p-1">+{{ $student->country_code }} {{ $student->phone }}</td>
                    <td class="p-1">{{ $student->last_degree }}</td>
                    <td class="p-1">
                    ...
                    <td class="p-1 text-center">
                        ...
                        <div class="d-flex justify-content-center align-items-center">
                            ...
                            <button class="btn btn-outline-primary mx-1 edit-stu" data-status-id="{{ $student->id }}" title="Edit" style="font-size: 0.5rem; padding: 0.1rem 0.2rem; border-width: 1px;">
                                <i class='bx bx-edit'></i>
                            </button>
                            <!-- Delete Button with Icon -->
                            <a href="{{ route('student.show', $student->id) }}"><button class="btn btn-outline-primary mx-1 view-profile-btn" title="Profile" style="font-size: 0.5rem; padding: 0.1rem 0.2rem; border-width: 1px;">
                                <i class='bx bx-chevron-right'></i> <!-- Right Arrow Icon -->
                            </button></a>
                            <!-- Delete Button with Icon -->
                            <button class="btn btn-outline-danger mx-1 delete-btn" data-form-action="{{ route('student.delete', $student->id) }}" title="delete"  style="font-size: 0.5rem; padding: 0.1rem 0.2rem; border-width: 1px;">
...
<form action="{{ route('student.save') }}" id="StuForm" method="POST" enctype="multipart/form-data">
    @csrf
...
<script>
      ...
      //open the offcanvas model 
        document.getElementById('AddStudent').addEventListener('click', function(event) {
            ...
            document.getElementById('lead-title').textContent = 'Add New Student';
